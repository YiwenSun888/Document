...
3.select project and testbench type
4.import the rig file 
5.choose the channel that you need to calculate
6.Run LPT
7.Export to excel
...
.. image:: images/ForLarge.png

4. import the rig file
^^^^^^^^^^^^^^^^^^^^^^

Press the Rig File (Step1) button,then set the control channel and duration

.. image:: images/ControlChannel.png

5. choose the channel that you need to calculate
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. image:: images/ChnSelect.png

6. Run LPT
^^^^^^^^^^

Press the Run LPT button
